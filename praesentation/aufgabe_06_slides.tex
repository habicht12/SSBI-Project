...
The paper describes max or mean pooling; top-$k$ is already in our baseline.
...
\begin{frame}{Same array: hard top-$k$ versus sigmoid selection}
...
\begin{frame}{Sample classification and joint optimization}

\begin{center}\begin{tikzpicture}[>=Stealth,every node/.style={align=center,font=\small}]
\node (a) at (0,0) {Cell responses\\$N\times K$};
\node[text=Teal] (b) at (3.3,0) {Soft Top Fraction\\$P\in\mathbb R^K$};
\node (c) at (6.7,0) {Linear head\\$\ell=VP+\gamma$};
\node (d) at (10,0) {Softmax\\$\widehat p(y\mid X)$};
\draw[->,thick] (a)--(b);\draw[->,thick] (b)--(c);\draw[->,thick] (c)--(d);
\draw[<-,Teal,thick] (0,-1)--(10,-1);
\node[fill=white,text=Teal] at (5,-1) {Gradients from cross-entropy + regularization};
\end{tikzpicture}\end{center}
\vspace{2mm}
\textbf{Jointly learned:} filter parameters, $\alpha_k$ and output weights.\\
Frequency analysis is performed \emph{after} training; it does not replace this head.
\takeaway{Sample labels supervise the cell representation without requiring labels for individual cells.}

\source{Shared project discussion; CellCnn principle: Arvaniti \& Claassen (2017).}
\end{frame}
...
\begin{frame}{Activation, pooling and population thresholds}
...
\begin{frame}{Comparison plan for both ReLU variants}

\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}p{.25\linewidth}p{.34\linewidth}p{.35\linewidth}@{}}\toprule
Cell filter & Pooling comparison & Interpretation\\\midrule
Quadratic-ReLU & Fixed top-1\,\% $\leftrightarrow$ Soft Top Fraction & Pooling effect with the same filter and L2 penalty.\\
Mahalanobis-ReLU & Fixed top-1\,\% $\leftrightarrow$ Soft Top Fraction & Pooling effect with the same geometry penalty.\\\bottomrule
\end{tabular}
\vspace{3mm}
\textbf{Same donor splits and population rule.} Filter selection and threshold calibration use no test data.\par
\vspace{2mm}
\small Mahalanobis-ReLU + Soft Top Fraction is a target model. The read learnable branch uses negative distances; its results do not establish the performance of this combination.
\takeaway{Control filter geometry, pooling and regularization separately to interpret any performance difference.}

\source{Proposed comparison; no new experimental results. See the accompanying notes for the implementation snapshot.}
\end{frame}
