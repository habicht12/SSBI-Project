...
cell-level labels. In this project we compare and try to improve CellCnn \citep{arvaniti2017cellcnn}, a supervised representation-learning method that uses donor-level labels to detect disease-associated cell subsets.
...
the first 29 PCs (91.5\,\% of the variance) as input to the non-linear methods below to reduce
dimensionality before the more expensive computations while retaining most of the variance.

\begin{figure}[h]
  \centering
  \includegraphics[width=.55\linewidth]{./figures/pca_expl_var.png}
  \caption{Cumulative explained variance (blue)  across the 36 displayed principal components. }
  \label{fig:pca_spectrum}
\end{figure}

Next, we swept the perplexity parameter of \tsne{} \citep{maaten2008tsne} (via scikit-learn
\citep{pedregosa2011sklearn}) over the values 5, 30, 50, 60, 70 and 100, computing \trust{} and
...
scores for perplexities 50--100 spanned about 0.0020, so we chose the
...
pairwise distances within a random subsample of 2{,}000 cells. While the first two measures capture \emph{local} structure (is each point's immediate neighbourhood
...
\begin{table}[h]
  ...
  \begin{minipage}{0.62\linewidth}
    \centering
    {\footnotesize\input{aufgabe_02/data/quality}}
  ...
  \begin{minipage}{0.34\linewidth}
    \centering
    {\footnotesize\input{aufgabe_02/data/pairwise}}
...
By this measure, PCA and \tsne{} are the closest match. By contrast, for local-neighbour
